...
import { Calendar, MapPin, Clock, User, ChevronLeft, Share2, Download, FileText, Printer } from "lucide-react"
...
const NoticeDetails = () => {
  ...
  const handleDownloadAttachment = (attachment: any, e: React.MouseEvent) => {
    e.stopPropagation()
  ...
  <div className="min-h-screen bg-white" style={{ fontFamily: "Poppins, sans-serif" }}>
    ...
    <div className="w-full max-w-[1440px] mx-auto px-30 py-16">
      ...
      <div className="pt-[90px] pb-[22px]">
        ...
        {/* Attachments Section - Only show if notice has attachments */}
        {notice.attachments && notice.attachments.length > 0 && (
          <div className="mb-[50px]">
            <h2 className="font-semibold text-[26px] leading-[39px] text-black mb-[32px]">Attachments</h2>

            <div className="flex gap-[24px] flex-wrap">
              {notice.attachments.map((attachment, index) => (
                <div key={index} className="w-[158px] h-[150px] border border-[#D1D5DB] rounded relative">
                  <div className="p-[10px]">
                    <div className="w-[38px] h-[34px] bg-[#F0E6FA] rounded-md flex items-center justify-center mb-[7px] mx-auto">
                      <FileText className="w-5 h-5 text-[#9333EA]" />
                    </div>

                    <p className="text-[10px] leading-[15px] text-black font-light text-center mb-[3px] px-2 line-clamp-2">
                      {attachment.name}
                    </p>

                    <p className="text-[8px] leading-3 text-[#B0AAAA] font-light text-center mb-[23px]">
                      {attachment.size}
                    </p>
