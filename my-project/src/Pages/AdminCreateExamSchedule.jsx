...
const AdminCreateExamSchedule = () => {
  ...
  const handleEdit = (row) => {
    ...
    setFormData({
      date: row.raw.date,
      start_time: row.raw.start_time,
      end_time: row.raw.end_time,
      course_id: row.raw.course_id,
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.date || !formData.start_time || !formData.end_time || !formData.course_id || !formData.room_no || !formData.invigilator || !formData.semester) {
      setMessage({ text: "Please fill in all fields.", type: "error" });
      return;
    }
    try {
      // Extract numeric part from course code (CSE or CSE- followed by 3-4 digits)
      const numericCourseId = formData.course_id.match(/(?:CSE-?)?(\d{3,4})/)?.[1] || formData.course_id.replace(/\D/g, '');
      console.log('Original course_id:', formData.course_id);
      console.log('Extracted numeric course_id:', numericCourseId);
      
      const payload = {
        date: formData.date,
        start_time: formData.start_time,
        end_time: formData.end_time,
        course_id: parseInt(numericCourseId),
        room_no: formData.room_no,
        invigilator: formData.invigilator,
        semester: formData.semester,
      };
      console.log('Final payload course_id:', payload.course_id);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center py-12">
      <div className="w-full max-w-xl bg-white rounded-lg shadow-lg p-8 mt-8 mb-8">
        <h2 className="text-3xl font-bold text-[#13274C] mb-6 text-center">{editingId ? "Update Exam Schedule" : "Create Exam Schedule"}</h2>
        {message.text && (
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-[#FFC300]">
              <tr>
                {['Date', 'Time', 'Course ID', 'Room No', 'Invigilator', 'Semester', 'Actions'].map((header) => (
                  <th
                    key={header}
                    scope="col"
              ...
              {examData.length === 0 ? (
                ...
                examData.map((row, index) => (
                  <tr key={row.id} className={index % 2 === 1 ? "bg-gray-50" : undefined}>
                    ...
                    <td className="px-4 py-3 text-left text-sm text-gray-900">{row.course_id}</td>
